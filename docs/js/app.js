/* ============================================================
   ...
   Progress + two note features:
     1) per-item personal notes (every phase)
     2) phase-4 "1:1 oplæring" workspace: links + free notes
   State is hydrated from window.PM_CONTENT and persisted to
   localStorage so progress survives reloads and new sessions
...
(() => {
  ...
  function loadState() {
    ...
    return { checks: {}, item_notes: {}, workspaces: {}, seq: 1 };
  }
  const state = loadState();
  let saveTimer = null;
  function saveState() {
    // debounced write — keeps autosave snappy under rapid typing
  ...
  // Persist immediately if the tab is hidden/closed mid-edit.
  ...
  function wsOf(phase) {
    if (!state.workspaces[phase]) state.workspaces[phase] = { notes: [], links: [] };
    const w = state.workspaces[phase];
    w.notes = w.notes || [];
    w.links = w.links || [];
    return w;
  }

  /* ---------- in-memory mirrors (decoupled from persisted state) ---------- */
  const checks = new Set(Object.keys(state.checks));
  const itemNotes = new Map(); // key -> {item_key,body,height,created_at,updated_at}
  for (const [k, v] of Object.entries(state.item_notes)) itemNotes.set(k, { ...v });
  const workspaces = {}; // phase_id -> {notes:[], links:[]}
  for (const [pid, w] of Object.entries(state.workspaces)) {
    workspaces[pid] = {
  ...
  /* ---------- local "API": same call signatures, localStorage-backed ----------
     Async so the rest of the wizard logic stays byte-for-byte identical to the
     server version. Each op mutates the persisted `state` and returns the same
     shape the FastAPI backend used to return. */
  async function api(method, url, body) {
    const path = url.replace(/^\/api\//, "");
    const parts = path.split("/");

    // POST /api/check
    if (method === "POST" && parts[0] === "check") {
      if (body.checked) state.checks[body.item_key] = true;
      else delete state.checks[body.item_key];
      saveState();
      return null;
    }

    // item-notes/{key}
    if (parts[0] === "item-notes") {
      const key = decodeURIComponent(parts.slice(1).join("/"));
      if (method === "DELETE") { delete state.item_notes[key]; saveState(); return null; }
      if (method === "PUT") {
        const text = body.body || "";
        if (!text.trim()) { delete state.item_notes[key]; saveState(); return null; }
        const existing = state.item_notes[key];
        const row = {
          item_key: key,
          body: text,
        ...
        state.item_notes[key] = row;
        saveState();
        return { ...row };
      }
    }

    // workspaces/{phase}/notes  |  workspaces/{phase}/links
    if (parts[0] === "workspaces") {
      const phase = parts[1];
      const sub = parts[2];
      const w = wsOf(phase);
      if (sub === "notes" && method === "POST") {
        if (w.notes.length >= LIMITS.max_free_notes)
          throw new Error("Du har nået grænsen for noter i dette område.");
        const n = {
          id: nextId(), phase_id: phase,
          title: body.title || "", body: body.body || "", height: body.height | 0,
          position: w.notes.length, created_at: nowISO(), updated_at: nowISO(),
        };
        w.notes.push(n); saveState(); return { ...n };
      }
      if (sub === "links" && method === "POST") {
        const u = (body.url || "").trim();
        if (!(u.startsWith("http://") || u.startsWith("https://")))
          throw new Error("URL skal starte med http:// eller https://");
        if (w.links.length >= LIMITS.max_links)
          throw new Error("Du har nået grænsen for links i dette område.");
        const ln = { id: nextId(), phase_id: phase, label: body.label || "", url: u, created_at: nowISO() };
        w.links.push(ln); saveState(); return { ...ln };
      }
    }
    ...
    if (parts[0] === "notes") {
      ...
      for (const w of Object.values(state.workspaces)) {
        ...
        if (i >= 0) {
          if (method === "DELETE") { w.notes.splice(i, 1); saveState(); return null; }
          if (method === "PUT") {
            const n = w.notes[i];
            if (body.title != null) n.title = body.title;
            if (body.body != null) n.body = body.body;
            n.height = body.height | 0;
            n.updated_at = nowISO();
            saveState(); return { ...n };
          }
        }
    ...
    // links/{id}
    if (parts[0] === "links" && method === "DELETE") {
      const id = parseInt(parts[1], 10);
      for (const w of Object.values(state.workspaces)) {
        const i = (w.links || []).findIndex((l) => l.id === id);
        if (i >= 0) { w.links.splice(i, 1); saveState(); return null; }
      }
      return null;
    }
  ...
  /* ============================================================
     ...
     ============================================================ */
  function init() {
    // fill welcome stats from content (keeps numbers in sync with data)
    const totalEl = $("#wcTotal"); if (totalEl) totalEl.textContent = TOTAL;
    const vidEl = $("#wcVideos");
    if (vidEl) {
      const v = CONTENT.video_count != null
        ? CONTENT.video_count
